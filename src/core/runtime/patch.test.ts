...
describe("patch", () => {
  ...
  describe("effects", () => {
    // The engine runs whatever effects the ctx carries, after the writes. A local,
    // touched-guarded effect stands in: when `panel` is touched, it drops version.*.
    ...
    it("runs effects after the writes — the effect sees the just-written state", () => {
      expect(
